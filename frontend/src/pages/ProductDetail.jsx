  ...
  return (
    <div className="max-w-5xl mx-auto px-4 py-12">
      <div className="grid md:grid-cols-2 gap-12 items-start">
        ...
        {/* Product Image Section */}
        <div className={`aspect-square rounded-3xl bg-sage-50 border border-sage-100 overflow-hidden shadow-sm flex items-center justify-center relative ${isOutOfStock ? 'grayscale opacity-75' : ''}`}>
          {product.image ? (
            <img src={product.image} alt={product.name} className="w-full h-full object-cover" />
          ) : (
            <span className="text-9xl">{product.type === 'floral' ? '🌸' : '🎁'}</span>
          ...
          {isOutOfStock && (
            <div className="absolute inset-0 flex items-center justify-center">
              <span className="bg-slate-900/80 text-white px-6 py-2 rounded-full font-bold text-lg backdrop-blur-sm">SOLD OUT</span>
            </div>
          )}
        </div>
        ...
        <div className="flex flex-col">
          ...
          <div className="mb-6">
            {isOutOfStock ? (
              <span className="text-red-500 font-bold text-sm uppercase tracking-tight">🚫 Currently Out of Stock</span>
            ) : product.stock < 5 ? (
              <span className="text-orange-600 font-bold text-sm uppercase tracking-tight">⚠️ Only {product.stock} left in stock - order soon!</span>
            ) : (
              <span className="text-emerald-600 font-bold text-sm uppercase tracking-tight">✅ In Stock ({product.stock} available)</span>
            )}
          </div>
          ...
          <div className="p-6 bg-sage-50 rounded-2xl border border-sage-100">
            <div className="flex flex-col gap-4">
              <div className="flex items-center justify-between">
                ...
                <div className={`flex items-center border border-sage-200 bg-white rounded-lg overflow-hidden ${isOutOfStock ? 'opacity-50 pointer-events-none' : ''}`}>
                  <button 
                    onClick={() => setQuantity(q => Math.max(1, q - 1))}
                    className="px-4 py-2 hover:bg-sage-50 text-sage-600 transition-colors border-r border-sage-200"
                  >−</button>
              ...
              <button
                ...
                className={`w-full py-4 rounded-xl font-bold text-lg transition-all shadow-lg active:scale-[0.98] ${
                  ...
                  : 'bg-sage-800 text-white hover:bg-sage-900 shadow-sage-200'
                }`}
              >
                {isOutOfStock 
                  ? 'Sold Out' 
